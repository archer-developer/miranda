...
`anthropic.go:259-323` ставит `cache_control` breakpoint на последний
system-блок — но это не основной путь трафика.

---

## 2. Почему нельзя просто скопировать anthropic.go's приём

Anthropic-подход (`anthropic.go:259-323`) — **мягкий**: breakpoint помечает
«всё до сюда — переиспользуй, если совпадёт», а если содержимое разошлось
(например, `buildSystemPrompt`, `../../internal/httpapi/agent_loop.go:430`,
вставляет новое `Текущее время пользователя` на каждый вызов
`buildSystemPrompt`, то есть на каждый новый ход *разговора*, хотя не на
каждую итерацию *одного* хода — `messages[0]` фиксируется один раз в
`orchestrator.go:428` и переиспользуется без изменений внутри всех до 15
итераций одного `runAgentLoop`) — Anthropic просто не находит совпадения и
молча считает по полной цене для несовпавшего хвоста. Ошибки нет, деградация
плавная.
...
вообще для этого запроса. Кэшировать весь `systemInstruction` целиком
(персона + текущее время + shared/personal память) не имеет смысла: время
меняется на каждый вызов `buildSystemPrompt`, то есть кэш нужно было бы
пересоздавать почти на каждый ход — цена пересоздания сама по себе
съедает выгоду.

**Решение: кэшировать только `Tools`, никогда не `SystemInstruction`.**
Массив схем тулов (в отличие от system-текста) абсолютно не зависит от
времени, пользователя или памяти — при данной конфигурации MCP-серверов он
побайтово одинаков для всех пользователей, всех разговоров, всех ходов,
...
`CachedContent`. `SystemInstruction` при этом остаётся как сегодня —
всегда пересылается заново, инлайн, без кэша (это меньшая часть токенов —
после `docs/adr/lazy-mcp-tool-loading.md` в списке `Tools` тоже станет
меньше на общем ходу, но именно `SystemInstruction` был и остаётся
маленьким по сравнению с 66 схемами тулов).
...
|---|---|---|
| `Tools` (схемы тулов) | Да, через `CachedContent`, per-API-key | Побайтово одинаковы для всех пользователей/ходов при неизменном конфиге — идеальный кандидат для «всё или ничего» ресурса |
| `SystemInstruction` (персона + время + память) | Нет, всегда инлайн | Меняется на каждый вызов `buildSystemPrompt` (время — на каждый ход разговора) — see §2 |
