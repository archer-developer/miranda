...
> **Обновление (2026-08-18):** механизм расширен на нативные (не-MCP)
> домены тулов — `google_calendar` (`internal/calendar`,
> `../../internal/httpapi/calendar_tools.go`, см. `docs/adr/oauth2-layer.md`)
> точно так же скрыт за `load_tool_group`, хотя за ним не стоит никакого
> `config.MCPServer`. В отличие от MCP-серверов, для которых `lazy` —
> настраиваемый флаг (`false` по умолчанию), `google_calendar` ленив
> безусловно, всегда, когда включён (`Orchestrator.calendarEnabled()`) —
> отдельного тумблера нет и не нужно, ровно по той же причине "релевантно
> лишь малой доле ходов", что и у `diary`/`yazio`/`medical_card`.
> `availableTools` объединяет описания MCP-серверов
> (`o.lazyServerDescriptions`) и календаря в один `groupDescriptions`, а
> `loadToolGroupStub` теперь принимает эту карту параметром вместо чтения
> поля оркестратора напрямую — единственное структурное изменение,
> позволившее добавить нативный домен без раздувания сигнатур.
...
mcp:
  servers:
    ...
    - name: medical_card
      ...
      expose_files: true
      lazy: true
      description: "Медкарта: вопросы по истории болезни, профиль здоровья (диагнозы, лекарства, аллергии, прививки, анализы, показатели), хронология событий, документы, самостоятельные записи о здоровье, напоминания о предстоящих действиях, снятие диагнозов."
