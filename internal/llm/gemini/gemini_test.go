...
// quotaErrorBody is a Gemini generateContent 429 response body, the same
// shape internal/tts/gemini.go's requestOnce already parses for its own
...
// the test (restored via t.Cleanup) — mirrors internal/tts/gemini_test.go's
// use of internal/tts's own geminiAPIBaseURL var. Returns a function that
// reports how many requests have been received so far.
...
func newTestProvider(t *testing.T, n int, script []scriptedResponse, rotation config.GeminiRotationConfig) (*Provider, func() int) {
	t.Helper()
	...
	p, err := New(context.Background(), "gemini-test", "gemini-test-model", envs, config.GeminiToolsConfig{}, rotation, nil)
	require.NoError(t, err)
...
func TestGeminiNew_NoKeysConfiguredErrors(t *testing.T) {
	_, err := New(context.Background(), "gemini-test", "gemini-test-model", []string{"GEMINI_TEST_UNSET_ENV_VAR"}, config.GeminiToolsConfig{}, config.GeminiRotationConfig{}, nil)
	require.Error(t, err)
}

func TestGeminiNew_ContextCachingRejected(t *testing.T) {
	t.Setenv("GEMINI_TEST_KEY", "fake-key")
	_, err := New(context.Background(), "gemini-test", "gemini-test-model", []string{"GEMINI_TEST_KEY"}, config.GeminiToolsConfig{ContextCaching: true}, config.GeminiRotationConfig{}, nil)
	require.Error(t, err)
...
func TestGemini_RotatesOnQuotaError(t *testing.T) {
	...
	p, requestCount := newTestProvider(t, 2, script, config.GeminiRotationConfig{})
...
func TestGemini_RotatesOnServerError(t *testing.T) {
	// This is the direct regression test for broadening rotation beyond
	// internal/tts/gemini.go's quota-only trigger: a plain 503 (no
	// RESOURCE_EXHAUSTED status) must still rotate to the next key, unlike
	// TTS's callGemini, which would return this error immediately without
	// rotating (see that function's doc comment). Don't "fix" this back to
	// match TTS's narrower behavior — it's intentional, per this adapter's
	// requirement.
	script := []scriptedResponse{
...
func TestGemini_AllKeysExhaustedAfterCycles(t *testing.T) {
	...
	p, requestCount := newTestProvider(t, 2, script, config.GeminiRotationConfig{MaxRetryCycles: 2, CooldownSeconds: 0})
...
func TestGemini_RotationEmitsNoChunksFromFailedAttempt(t *testing.T) {
	...
	p, _ := newTestProvider(t, 2, script, config.GeminiRotationConfig{})
...
func TestGemini_ForwardsTextChunksLiveAsTheyArrive(t *testing.T) {
	...
	p, _ := newTestProvider(t, 1, script, config.GeminiRotationConfig{})
...
// in production, already spoken via TTS).
...
// ProviderMetadata (as the orchestrator would replay it from
// history.ToolCallRef on a later turn) must produce a genai.Part with
// ThoughtSignature set on the reconstructed FunctionCall part.
