...
// an escalation hop (e.g. gemini-lite's own earlier tool calls, replayed to
// gemini-strong; a signature is only ever verified against the exact model
// that emitted it) — falls back to geminiThoughtSignaturePlaceholder
// instead of an empty value, which hits the identical 400 in production
// (confirmed via llm.log: an "error-escalation" synthetic call reaching
// gemini-strong). Google's own docs for this exact case (a function call
// "executed deterministically by the client" or transferred from a
// different model) name this literal sentinel as the documented bypass —
// see https://ai.google.dev/gemini-api/docs/generate-content/thought-signatures.
// geminiThoughtSignaturePlaceholder is Google's documented dummy
// thought_signature value for a FunctionCall part that wasn't actually
// generated by the model — see toGeminiContents' doc comment above. Passed
// as raw bytes through genai.Part.ThoughtSignature ([]byte), which the SDK
// base64-encodes on the wire like any real signature; Google's docs also
// name "skip_thought_signature_validator" as an equivalent alternative.
		...
		case llm.RoleAssistant:
			...
			for _, tc := range m.ToolCalls {
				...
				if tc.ProviderMetadata != "" {
					...
					// all — mirrors the Arguments unmarshal above, except an
					// empty signature is a hard 400 here, not a silently
					// degraded field, so there's no safe empty fallback.
