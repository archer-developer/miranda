// Vendored copy of lit-html's core runtime (html/svg tag functions +
// render()) — a lightweight, safe templating helper used by the chat
// screen (see ../../screens/chat.js, ../../segment-template.js) to render
// server-parsed reply markup (bold/italic/code/link/list — see
// internal/replyformat) without hand-rolled innerHTML template literals or
// manual document.createElement calls.
//
//   Source:  https://lit.dev / https://github.com/lit/lit
//   Package: lit-html@3.3.3 (npm), bundled via esm.sh's ES2022 bundler
//            (https://esm.sh/lit-html@3.3.3/es2022/lit-html.bundle.mjs)
//   License: BSD-3-Clause (see ./LICENSE in this directory — copyright
//            notice preserved there per the license's terms)
//
// Why vendored instead of `npm install`-ed or pulled from a CDN at runtime:
// same reasoning as ../json-formatter-js/json-formatter.mjs — this project
// has no Node/npm dependency and no bundler, compiles to one static Go
// binary via `go:embed`, and is a self-hosted LAN app that shouldn't need
// outside network availability to load its own dashboard.
//
// Why one flattened bundle instead of vendoring lit-html's own npm dist
// verbatim: unlike json-formatter-js (already a single prebuilt file),
// lit-html's npm package ships as several small ESM files importing each
// other by relative path — committing that whole tree would work but
// scatters "vendored, don't hand-edit" across many files instead of one.
// esm.sh's per-module bundler flattens the whole dependency graph (reactive
// element internals, template processing, directive plumbing) into this
// single file — verified zero further imports/dynamic imports before
// vendoring (grep for "^import"/"import(" turns up nothing below).
//
// Deliberately does NOT include lit-html's directives/unsafe-html module:
// that submodule pulls in its own copy of lit-html's internal Directive
// machinery, and bundling it as a second, separately-fetched esm.sh file
// would instantiate a *second* copy of lit-html's core alongside this one
// — two independent module graphs whose internal directive-tagging can't
// be trusted to interoperate. Not needed anyway: lit-html accepts a real
// DOM Node as a template child value directly (committed lit-html
// behavior, not a directive), so trusted, hardcoded content — e.g.
// icons.js's own SVG strings, the same "our own registry, not user input"
// trust boundary every innerHTML call site in this codebase already
// relies on — is embedded by building a real Node and interpolating that,
// not via unsafeHTML(). See segment-template.js's icon handling.
//
// Kept byte-for-byte identical to the upstream bundle (including its
// minification) rather than reformatted, so this file stays diffable
// against a future re-fetch of a newer lit-html release. Do not hand-edit
// the code below — update by re-vendoring instead (re-run the esm.sh fetch
// against a newer lit-html@x.y.z and replace everything past this header).
/* esm.sh - lit-html@3.3.3 */
