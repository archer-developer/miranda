...
// shared by internal/tts's Gemini TTS provider and internal/llm/gemini's
// chat provider — both rotate across multiple Gemini API keys (free-tier
// keys have a low per-key quota) using the identical cycle/cooldown shape,
// differing only in what one attempt actually does and which errors are
// worth rotating for.
package keyrotation
...
// errors.Is against whatever errKeyQuotaExceeded-style sentinel lastErr
// itself wraps, via %w), for a message that names how much was attempted.
//
...
// knows enough to log a domain-specific message (e.g. "tts: gemini:" vs
// "gemini:" prefixes, or fields like model/voice only the caller has).
