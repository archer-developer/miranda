...
// its own request/response dump (see internal/llm.Tracer) — this package
// only owns the shared block framing (timestamp, provider name,
// conversation id) and serializing writes to the underlying file, so every
// provider's trace looks the same at a glance no matter which SDK produced
// it, and — critically — reflects exactly what that SDK actually sent and
// received rather than a reconstruction from the provider-agnostic
// llm.ChatRequest, which can't represent provider-specific specifics (e.g.
...
// *Logger is valid and Trace on it is a no-op, so tracing can be wired up
// as an optional dependency (see router.Router.SetTracer and each
// Provider's own SetTracer).
type Logger struct {
	mu sync.Mutex
	w  io.Writer
}
...
// Trace implements internal/llm.Tracer. request and response are already
// fully formed by the calling Provider, verbatim — this Logger doesn't
// interpret their content, just frames and writes them. response is empty
// when err is non-nil. Safe for concurrent use — writes are serialized so
// concurrent turns can't interleave mid-block.
